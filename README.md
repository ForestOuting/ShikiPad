...
为避免多个模块在同一个约 1 ms 轮询帧中同时向 Interception 发送独立输出，离散输出使用单通道仲裁。优先级从高到低为：
...
旧的长按后滑识别，以及显示器移动、`Home`、`End` 手势均已删除。`Win + Shift + M` 和 `Win + M` 现在只作为二指右区直接滑的上/下映射。
