...
Two-finger continuation: after a two-finger swipe has been recognized, if the still finger stays within `TouchGestureHoldStillDistance`, ShikiPad keeps that two-finger state alive. The moving finger can keep sliding or lift and touch again, and each later segment still uses the normal two-finger ownership rule: the moving finger's segment start, current position, and direction decide the side, not the still finger. Continuation does not change each shortcut's mapping or repeat mode: right-side up/down remains timed-repeat `Ctrl + Shift + Tab` / `Ctrl + Tab`; right-side left/right remains single-shot `Alt + ←/→`; the left side still uses the left two-finger mapping, such as right swipe `Alt + F4`. Single-shot actions do not repeat until the moving finger lifts and swipes again. Each moving contact decides direct-vs-hold independently: the decision is made at that contact's first trigger, stays fixed while that finger remains down, and is recalculated only after it lifts and touches again. If the moving finger leaves while the still finger remains still, ShikiPad keeps the two-finger continuation alive and waits for the second finger to return. If the remaining still finger starts moving instead, it is reseeded as a one-finger gesture from the position and time when the other finger left, and direct-vs-hold is measured from that moment. If the still finger starts moving while the other finger is still down, no new two-finger gesture is fired; the current two-finger continuation ends until the touch points are released.
