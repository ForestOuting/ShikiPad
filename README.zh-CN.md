...
如果觉得手柄打字仍然太难, 建议搭配 Typeless, 闪电说等语音输入软件. DualSense 在这里很顺手: Create 是 `Right Alt`, Options 是 `Right Ctrl`, Home 负责蓄力, 静音键短按打开一次性 Fn 层, 静音键长按切换启用 / 禁用 ShikiPad.
...
静音键负责手柄 Fn 层. 短按静音键会打开一次性 Fn: 未带 Shift 的动作键映射 `1..0`, `-`, `=` 会变成 `F1..F12`. 下一次动作键无论是否转换成 F 键都会清空 Fn; 如果动作键不是这 12 个键之一, 就按原本映射正常输出. 长按静音键使用和 Home 蓄力相同的时间 `ClutchLongPressMs`, 用于切换 ShikiPad 启用 / 禁用.
