...
| `comboLayerWindowMs` | 80 | R1/R2 或 L1/L2 被视为组合层的最大间隔。它和动作键层确认窗口对齐，既给有意组合留出空间，又避免动作键已经落定后旧肩键/扳机的晚重叠误进组合层。 |
...
如果要调肩键/扳机组合层的判定时间，修改 `shikipad.json` 里的 `comboLayerWindowMs`，保存后重启 ShikiPad。比较实用的范围是 70-120 ms：调小更利于快速切换单层，调大更容易按出组合层。默认值和 `actionLayerGraceMs` 对齐，可以避免动作键层已经落定后仍把晚重叠识别成组合层。
...
## Interception 内核驱动模式（解决虚拟机和游戏拦截）

默认情况下，ShikiPad 使用 Windows 的 `SendInput` 注入虚拟按键。但这会被虚拟机（VMware/VirtualBox）和一些带有底层反作弊的游戏拦截，导致在这些软件中按键失灵。

为此，ShikiPad 集成了 **Interception** 驱动，可以在操作系统的驱动层注入硬件级扫描码和鼠标信号。

### 如何启用：

1. 双击运行 `install_driver.bat`，或在管理员命令行下执行 `install-interception.exe /install`。
2. **必须重启电脑**才能使驱动生效。
3. 确保 `shikipad.json` 中的 `useInterception` 为 `true`（默认即为 true）。
4. 重启 ShikiPad。此时你的手柄输入将在所有虚拟机和游戏中生效。
