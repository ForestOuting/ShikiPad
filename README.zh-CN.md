...
蓄力激活时, 已收集的修饰键会继续保持, 即使左摇杆离开原来的方向也不会丢失. 这样可以先保留一个修饰键, 再把摇杆推到上/下触发滚轮, 或转到其他修饰键方向继续叠加. 短按锁定的蓄力会在任意动作键实际发出后自动解除; 长按保持的蓄力仍然按住保持, 松开释放.

| 手柄 | 开启 / 保持 |
|---|---|
| DualSense / DualShock 4 | 触控板短按锁定到下一次动作键, 长按保持 |
| Xbox | View/Back 或 Menu/Start 短按锁定到下一次动作键, 长按保持 |
