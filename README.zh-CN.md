...
当前版本使用程序内置默认参数，不读取 `shikipad.json`。旧版本留下的配置文件可以忽略。
...
选择后 ShikiPad 会在后台运行。关闭 ShikiPad 控制台窗口即可退出，程序会自动释放所有保持中的键盘和鼠标按键。运行中按 `Enter` 可以展开完整映射说明。

## 紧急开关

同时按住 `Share/Create/View` 和 `Options/Menu` 持续 2 秒，可以切换 ShikiPad 启用/禁用。需要暂时恢复手柄原始输入时使用这个开关。
...
| 命令 | 用途 |
|---|---|
| `ShikiPad.exe --controller ds5` | 直接使用指定手柄型号启动 |
| `ShikiPad.exe --controller xboxseries` | 可用别名包括 `ds5`、`ds5bt`、`ds4`、`ds4bt`、`xbox360`、`xbox360bt`、`xboxseries`、`xboxseriesbt` |
