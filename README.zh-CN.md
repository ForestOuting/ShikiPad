...
ShikiPad 的控制台界面分为三类页面。

| 页面 | 何时出现 | 你需要做什么 |
|---|---|---|
| 初始页面 | 没有默认手柄，或你从主界面按 `Esc` 返回时 | 输入 `1` 到 `8`，按 `Enter` 确认；按 `Esc` 退出 |
| 欢迎页面 | 手柄连接成功后 | 按 `Enter` 打开映射说明；按 `Esc` 返回初始页面 |
| 映射说明页 | 运行中按 `Enter` 后 | 按 `Esc` 关闭软件 |
...
已有默认手柄时，ShikiPad 不再显示默认启动页，会直接使用保存的型号启动并等待手柄连接。

- 在欢迎页面按 `Esc`：返回初始页面，可重新选择本次要使用的手柄。
- 在初始页面按 `Esc`：退出 ShikiPad。
...
| PS 静音键 | `Caps Lock` 大写锁定 | 仅 DualSense (PS5) 手柄 |
...
基础层按住会连发。字符层是虚拟点按：按下一次只发送一个字符，按住不会连发。
由于部分跨边组合键（如 L1+R2、L2+R1）在人体工学上更难完全同步按下，系统为这两种特定组合分配了额外的 +10ms 容错时间，以保障更纯净的触发体验。
低频 Shift 副符号不单独占键：左摇杆推左保持 `Shift` 后，按右手图案键上的 `,`、`.`、`-` 等基础键，可输入 `<`、`>`、`_` 等副符号。
...
先确认已安装驱动并重启 Windows，然后以管理员权限运行 ShikiPad。当前版本强制使用 Interception；如果 Interception 不可用，程序会停止启动并提示安装/重启/管理员权限，不会回退到 `SendInput`。
