...
1. 动作键按下后，会先进入 `actionLayerGraceMs` 的短确认窗口。
2. 在确认窗口内，如果动作键仍然按着，程序会记录出现过的最新非基础层。
3. 如果最终确认到字母、数字、标点层，就发送一次虚拟 tap：`KeyDown + KeyUp`。
4. 虚拟 tap 发送后，这个动作键在本次按住期间被消费，不会再因为肩键/扳机变化变成另一个字符。
5. 如果最终确认到基础层，就进入真实按住模式，并启用渐进式连发。
6. `R1+R2` 和 `L1+L2` 组合层会锁定本次动作键。组合键先松开一个时，不会回落到 `R1/R2/L1/L2` 单层。
7. `R1+R2` 和 `L1+L2` 只有在第二个键进入 `comboLayerWindowMs` 时间窗口内才算组合层。如果某个肩键或扳机已经按住超过这个窗口，之后再和对应键短暂重叠，会按最后触发的单层处理，不会误变成组合层。
8. 动作键一旦松开，pending 状态不会再继续捕获下一组肩键/扳机，避免上一枚键污染下一枚键。

实际效果应该是：

- 快速 `R1 + 右`，再快速 `L1 + 方块`，输出 `nd`。
- 快速 `L1 + 上`，再快速 `R1 + 叉`，输出 `sh`。
...
| 参数 | 默认值 | 说明 |
|---|---:|---|
| `actionLayerGraceMs` | 80 | 动作键按下后的层确认窗口。快速打字掉回 Space/Tab 时可以略微增大；觉得字符输出太慢时可以略微减小。 |
