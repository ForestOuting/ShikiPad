...
| 模式 | ShikiPad 支持 | 限制 |
|---|---|---|
| DualSense USB | 完整 PlayStation 功能: 按键、摇杆、扳机、Home、触控板按压/手势、Create/Options、DualSense 静音键 | 蓝牙不稳定时优先用 USB |
| DualSense 蓝牙 | 只有 Windows 提供增强 `0x31` HID 报告时才是完整功能 | 如果系统或蓝牙栈只给简化 `0x01` 报告, ShikiPad 只能读普通按键/摇杆/扳机/Home, 不能读触控板坐标和 DualSense 静音键; 这属于系统/蓝牙 HID 报告限制, 建议改用 USB、重新配对/重连手柄, 或更新/更换蓝牙适配器和驱动 |
| DualShock 4 USB / 蓝牙 | 按键、摇杆、扳机、Home、触控板按压/手势、Share/Options | 没有 DualSense 静音键 |
| Xbox / XInput | 普通 XInput 按键、摇杆、扳机、View/Back、Menu/Start | 没有触控板; Guide/Home 通常被 Windows 保留, XInput 读不到; HidHide 通常也不能在 HID 层隐藏 XInput 设备 |
...
如果觉得手柄打字仍然太难, 建议搭配 Typeless, 闪电说等语音输入软件. PlayStation 手柄在这里很顺手: Share 是 `Right Alt`, Options/Menu 是 `Right Ctrl`, Home 根据按下时的状态负责蓄力或真实 `Left Shift`, DualSense 静音键按下即切换启用 / 禁用 ShikiPad. 支持麦克风的 PlayStation 手柄离嘴比较近, 安静环境下也能有不错的识别效果.
...
| 手柄 | 开启 / 保持 |
|---|---|
| DualSense / DualShock 4 | 先触发左摇杆修饰键再按 Home: 短按锁定到下一次动作键, 再短按取消, 长按保持; 没有修饰键时按 Home 就是真实 `Left Shift` |
| Xbox | Guide/Home 通过 XInput 通常无法读取, 因此跳过 |
...
程序发送的是物理键. 需要 Shift 的按键 (", :, \|, ~) 会由对应键层自动发送 Shift; PlayStation 手柄在没有左摇杆修饰键时按住 Home 也会按住真实 `Left Shift`.
