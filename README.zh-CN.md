...
触控板手势只在 PlayStation 手柄上可用. 当前规则是: 从触摸起点移动到 `TouchGestureThreshold` 后, 比较左 / 右 / 上 / 下四个方向分量, 哪个方向占比最大就识别为哪个方向. 没达到阈值不会触发快捷键.

### 触控板映射

| 手势 | 上 | 下 | 左 | 右 |
|---|---|---|---|---|
| 一指直接滑 | `Alt + Shift + Esc` 上一个窗口 | `Alt + Esc` 下一个窗口 | `Shift + Win + S` 截图 | `Alt + F4` 关闭软件 |
| 一指长按后滑 | `Ctrl + Shift + Tab` 上一个标签页 | `Ctrl + Tab` 下一个标签页 | `Alt + ←` 后退 | `Alt + →` 前进 |
| 二指直接滑 | `Win + Ctrl + ←` 前一个窗口 | `Win + Ctrl + →` 后一个窗口 | `Delete` 删除 | `Ctrl + Shift + Esc` 任务管理器 |
| 二指长按后滑 | `Home` | `End` | `Win + Shift + ←` 移到左显示器 | `Win + Shift + →` 移到右显示器 |

除了一指直接左滑截图、一指直接右滑关闭软件、二指直接右滑任务管理器之外, 其余触控板手势都会在手指保持在触控板上且方向仍满足阈值时连发. 连发间隔由 `TouchGestureRepeatMs` 决定.
...
左摇杆滚轮速度按半径连续变化: 当前上/下扇区决定滚轮方向, 距离圆心的远近会驱动指数 1.5 的几何间隔曲线, 从 1500ms 慢速滚轮过渡到 15ms 快速滚轮.
...
| `MouseScrollCurveExponent` | 1.5 | 左摇杆滚轮半径曲线指数 |
| `ScrollSlowIntervalMs` | 1500 ms | 最慢滚轮间隔 |
| `ScrollFastIntervalMs` | 15 ms | 最快滚轮间隔 |
| `WheelDelta` | 120 | 一个标准滚轮格 |
| `WheelQuantum` | 4 | 滚轮输出最小量化单位, 30 个单位等于一个标准滚轮格 120 |
| `MaxWheelDeltaPerFrame` | 120 | 单帧最多输出的滚轮量 |

左摇杆滚轮和右摇杆类似, 也是连续速度积分后量化输出. 半径先归一化为 `(radius - LeftStickEnterDeadzone) / (1 - LeftStickEnterDeadzone)`, 再计算 `power = normalized ^ MouseScrollCurveExponent`. 滚轮间隔使用几何插值: `interval = ScrollSlowIntervalMs * (ScrollFastIntervalMs / ScrollSlowIntervalMs) ^ power`, 速度为 `WheelDelta * 1000 / interval`. 程序累计小数滚轮量, 达到半个 `WheelQuantum` 后按 4 的倍数量化输出, 每帧最多输出 120.
